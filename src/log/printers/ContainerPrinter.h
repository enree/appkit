...
std::basic_ostream<charT, traits>& operator << (std::basic_ostream<charT, traits>& stream,
    const std::pair<First, Second>& _pair)
{
    stream <<
        (boost::format("(%1$s, %2$s)") % _pair.first % _pair.second).str();
    return stream;
}
...
public:
    ...
    InfixOstreamIterator(ostream_type& s)
        : m_os(&s), m_delimiter(0), m_first_elem(true)
    {}
...
template<typename Container> inline std::ostream&
print (std::ostream& strm
    , const Container& container, const std::string& name)
{
    strm << name << "(";
