...
public:
    ...
    connect(const typename SignalType::slot_type& subscriber)
    {
...
template <class SignalType>
void singleShot(
    ConnectHelper<SignalType> helper,
    const typename SignalType::slot_type& subscriber)
{
    BOOST_AUTO(connection, helper.connect(subscriber));
    helper.connect(
        boost::bind(&boost::signals2::connection::disconnect, connection));
}
...
public:                                                                        \
    typedef boost::signals2::signal<SIGNATURE> NAME##Signal;                   \
                                                                               \
protected:                                                                     \
    mutable NAME##Signal m_##NAME;                                             \
                                                                               \
public:                                                                        \
    boost::signals2::connection on##NAME(                                      \
        TYPENAME NAME##Signal::slot_type subscriber,                           \
        const boost::weak_ptr<void>& trackedObject) const                      \
    {                                                                          \
...
#define ADD_SIGNAL_T(NAME, SIGNATURE)                                          \
    ADD_SIGNAL_HELPER(NAME, SIGNATURE, typename)
