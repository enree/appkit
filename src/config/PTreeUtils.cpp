...
void validate(
    const boost::property_tree::ptree& ptree, const std::set<std::string>& keys)
{
    ...
    const std::string& parent)
{
    ...
    for (ptree::const_iterator ptreeIt = ptree.begin(); ptreeIt != ptree.end();
         ++ptreeIt)
    {
        ...
        std::string newPath
            = parent.empty() ? ptreeIt->first : parent + '.' + ptreeIt->first;
                ...
                if (!keychain.empty())
                {
                    ...
                    else
                    {
                        ...
                        array.push_back(std::make_pair(
                            "",
                            boost::property_tree::ptree(child.second.data())));
                        continue;
                    }
                }
                else
