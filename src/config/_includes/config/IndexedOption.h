...
public:
    ...
    std::set<std::string> keys() const override
    {
        ...
        {
            allKeys.insert((boost::format("%1$s%2$d.%3$s") % m_parentPath % i
                            % m_childPath)
                               .str());
        }
...
private:
    ...
    IndexedOption(
        ...
        , m_setter(setter)
        , m_maxIndex(9) // Maximum 9 nodes (by default)
    {
    ...
    void updatePaths(char placeholder)
    {
        ...
        if (placeholderPosition != std::string::npos)
        {
            ...
            m_childPath
                = k.substr(placeholderPosition + 2); // 2 = placeholder + dot
        }
    }
    ...
    void read(const boost::property_tree::ptree& node) override
    {
        ...
        {
            std::string subKey
                = (boost::format("%1$s%2$d") % m_parentPath % idx).str();
            ...
            catch (const boost::property_tree::ptree_bad_data& ex)
            {
                std::vector<std::string> keysInfo(
                    1,
                    (boost::format("%1$s.%2$s") % subKey % m_childPath).str());
