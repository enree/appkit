...
public:
    ConfigParserTest()
        : m_settings(config::fromFile<config::format::JSON>(
...
TEST_F(ConfigParserTest, printUsage)
{
    ...
    subtreeParser
        .addOption("some_node_with_long_value", "Long value option", value);
...
{
    boost::property_tree::ptree m_settings
        = config::fromFile<config::format::JSON>(
            appkit::configFile("arrayconfig.json"));
...
TEST(ConfigParserWithArrayTest, readComplexArray)
{
    ...
    EXPECT_CALL(testIndexedArrayMock, addId(0, 22))
        .Times(testing::Exactly((1)));
    EXPECT_CALL(testIndexedArrayMock, addId(1, 33))
        .Times(testing::Exactly((1)));
